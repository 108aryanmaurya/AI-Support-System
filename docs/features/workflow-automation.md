...
- **`sla_warning` + `notify`:** dedicated SLA breach email (`notify.sla_warning` job). `staff` → org admin; `assignee` → current assignee when set. Other triggers still use inbound-style staff notify.
