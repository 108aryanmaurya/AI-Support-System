-- Remove `waiting_customer` from conversations.status (use waiting_status instead).

update public.conversations
set
  waiting_status = case
    when coalesce(waiting_status, '') = '' then 'waiting_customer'
    else waiting_status
  end,
  status = 'open'
...
alter table public.conversations
  ...
  check (
    status in ('open', 'pending', 'resolved', 'closed', 'spam')
  );

comment on column public.conversations.status is
  'Lifecycle: open, pending, resolved, closed, spam. Who-waits is waiting_status.';
...
create index if not exists idx_conversations_org_waiting_agent_sla
  on public.conversations (organization_id, last_customer_message_at)
  where waiting_status = 'waiting_agent' and status in ('open', 'pending');

-- Align handle_incoming_message active-thread lookup (open / pending only).
